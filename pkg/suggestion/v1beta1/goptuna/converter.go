...
func toGoptunaAlgorithmOpt(algorithm *api_v1_beta1.AlgorithmSpec) (goptuna.StudyOption, error) {
		...
		for _, s := range algorithm.GetAlgorithmSettings() {
			...
			case "restart_strategy":
				if s.Value == "ipop" {
					...
					return nil, fmt.Errorf("invalid restart_strategy: '%s'", s.Value)
				}
			}
		}
		return goptuna.StudyOptionRelativeSampler(cmaes.NewSampler(opts...)), nil
	case AlgorithmTPE:
		opts := make([]tpe.SamplerOption, 0, len(algorithm.GetAlgorithmSettings()))
		for _, s := range algorithm.GetAlgorithmSettings() {
			switch s.Name {
			case "random_state":
				seed, err := strconv.ParseInt(s.Value, 10, 64)
				if err != nil {
					return nil, fmt.Errorf("invalid settings %s: %w", s.Name, err)
				}
				opts = append(opts, tpe.SamplerOptionSeed(seed))
			case "n_startup_trials":
				n, err := strconv.Atoi(s.Value)
				if err != nil {
					return nil, fmt.Errorf("invalid settings %s: %w", s.Name, err)
				}
				opts = append(opts, tpe.SamplerOptionNumberOfStartupTrials(n))
			case "n_ei_candidates":
				n, err := strconv.Atoi(s.Value)
				if err != nil {
					return nil, fmt.Errorf("invalid settings %s: %w", s.Name, err)
				}
		...
		return goptuna.StudyOptionSampler(tpe.NewSampler(opts...)), nil
	case AlgorithmSobol:
		return goptuna.StudyOptionRelativeSampler(sobol.NewSampler()), nil
	case AlgorithmASAH:
		var opts []successivehalving.Option
		for _, s := range algorithm.AlgorithmSettings {
			switch s.Name {
			case "min_resource":
				n, err := strconv.Atoi(s.Value)
				if err != nil {
				...
				opts = append(opts, successivehalving.OptionSetMinResource(n))
			case "reduction_factor":
				n, err := strconv.Atoi(s.Value)
				if err != nil {
					return nil, fmt.Errorf("invalid settings %s: %w", s.Name, err)
				}
				opts = append(opts, successivehalving.OptionSetReductionFactor(n))
			case "min_early_stopping_rate":
				n, err := strconv.Atoi(s.Value)
				if err != nil {
					return nil, fmt.Errorf("invalid settings %s: %w", s.Name, err)
				}
				opts = append(opts, successivehalving.OptionSetMinEarlyStoppingRate(n))
			}
		}
		pruner, err := successivehalving.NewPruner(opts...)
		if err != nil {
			return nil, err
		}
		return goptuna.StudyOptionPruner(pruner), nil
	default:
		fallthrough
	case AlgorithmRandom:
		...
		for _, s := range algorithm.GetAlgorithmSettings() {
			if s.Name == "random_state" {
				...
				opts = append(opts, goptuna.RandomSamplerOptionSeed(seed))
			}
		}
		return goptuna.StudyOptionSampler(goptuna.NewRandomSampler(opts...)), nil
	}
}

func toGoptunaSearchSpace(parameters []*api_v1_beta1.ParameterSpec) (map[string]any, error) {
	searchSpace := make(map[string]any, len(parameters))
...
func toGoptunaParams(
	...
	externalParams = make(map[string]any, len(assignments))
...
func createStudyAndSearchSpace(experiment *api_v1_beta1.Experiment) (*goptuna.Study, map[string]any, error) {
	...
	algorithmOpt, err := toGoptunaAlgorithmOpt(experiment.GetSpec().GetAlgorithm())
	if err != nil {
		return nil, nil, err
	}
	...
	study, err := goptuna.CreateStudy(
		...
		goptuna.StudyOptionDefineSearchSpace(searchSpace),
